(function () {
    ...
    var a = function (r, C) {
        ...
        var s = Math.ceil(B / 7);
        var z = toTwo(w) + s + q;
        var v = "t" + toTwo(C.lunarMonth) + toTwo(C.lunarDay);
        var t = toTwo(w) + toTwo(B);
        var x = [];
        var D;
    ...
    var f = function (r) {
        ...
        return {
            //p是生肖
            animal: ANIMAL[(x.lunarYear - 4) % 12],
            //干支日
            gzDate: gz.getGzDay(r),
            //干支月
            gzMonth: gz.getGzMonth(r, year, month),
            //干支年
            gzYear: gz.getGzYear(r, year, x.lunarYear),
            //农历的年数字的方式
    ...
    /*A.ui.lunar = function(q) {
     var q = new Date(q);
     return f(q)
     }*/
    var cni = new Date();
    cni.setFullYear(2016, 11, 8);
    console.log(f(cni));
})();